...
#' @import ggplot2
#' @importFrom reshape2 melt
group_traceplot <- function(output) {
  ...
  ggplot(out_df, aes(x = .data$iteration, y = .data$estimate)) +
    geom_line(aes(colour = .data$group)) +
...
group_autocorrplot <- function(output, lagmax) {
  ...
  ac_df <- melt(autocorr, varnames = var_names, value.name = "autocorrelation")
  ac_df$lag <- ac_df$lag - 1
  ac_df$group <- factor(ac_df$group)
  
  ggplot(ac_df, aes(x = .data$lag, y = .data$autocorrelation)) +
    geom_bar(aes(fill = .data$group), position = position_dodge(),
